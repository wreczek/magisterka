...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{rys.31.confusion.matrix.better.quality.png}  %rys.31.confusion.matrix.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.35\textwidth]{rys.33.diagram.kolowy.legenda.png} % 52
...
\begin{itemize}
	\item w~pierwotnym zbiorze wystąpiły dwa zdjęcia (przykładowo rys. \ref{fig:mesh15}, ich nazwy to \ita{1\_1E\_t\_500x} oraz \ita{1\_4E\_t\_500x}), które nie nadawały się do tej analizy, gdyż nie zawierały w~sobie głównych form grafitowych (rys. \ref{fig:mesh14})
...
\begin{table}[h]
	...
	\end{threeparttable}
\end{table}
Następnie te \ita{k} rezultatów jest uśrednianych. Zastosowano domyślne wartości parametrów bez wykorzystania strojenia parametrów, gdyż wyniki te są o wiele gorsze niż w przypadku innych metod.
...
\begin{table}[h]
	...
	\end{threeparttable}
\end{table}
Dodatkowo, aby nie zwiększać sztucznie dokładności ze względu na podobieństwo danych wygenerowanych sztucznie i~oryginalnych, postanowiono nałożyć szum na dane wygenerowane syntetycznie. Zdecydowano się na szum biały. Wykorzystano bibliotekę \ita{scikit-image} a precyzyjniej funkcję \ita{scikit-image.util.random\_noise} z domyślnymi wartościami parametrów. Wyniki zostały przedstawione w~tabeli \ref{hu.haralick.table.with.augmentation.and.noise}.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{rys.41.flowchart.diagram.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{rys.46.decision.tree.depth.2}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{rys.47.cart.vs.id3.czas.uczenia}
    \caption{Wykres przedstawiający średni czas uczenia modeli skonstruowanych przy pomocy różnych algorytmów generowania drzewa, w~zależności od jego głębokości. Źródło: opracowanie własne (z wykorzystaniem biblioteki \ita{matplotlib})}
...
\begin{figure}[h]
    ...
    \caption{Wykres przedstawiający średni czas uzyskania wyniku przez modele skonstruowane przy pomocy różnych algorytmów generowania drzewa, w~zależności od jego głębokości. Źródło: opracowanie własne (z wykorzystaniem biblioteki \ita{matplotlib})}
...
Na koniec, zanim przedstawimy tabelę podsumowująca wszystkie testy związane z~drzewem decyzyjnym, przyjrzymy się jeszcze wynikom osiąganym za pomocą drzewa decyzyjnego skonstruowanego przy pomocy algorytmu CART, gdy uprzednio zbalansujemy dane. Tym razem zdecydowano się na proste usunięcie danych z klasy nadreprezentowanej (przykłady o wysokiej wytrzymałości na rozciąganie).
\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{rys.50.decision.tree.depth2.balanced}
...
Jako podsumowanie prac z~drzewami decyzyjnymi zostaje przedstawiony szkic (tab. \ref{dt.binary.summary.table}), który zawiera wyniki większości testów wartych uwagi , również tych, które nie zostały tutaj bliżej opisane.
\begin{table}[h]
	\centering
	\begin{threeparttable}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.45\textwidth]{rys.56.fit.times}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.45\textwidth]{rys.57.score.times}
